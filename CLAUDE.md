...
- **v0.3.0** on branch `week3-narratives-and-review`, ready for the PR into `main`. Weeks 1–3
  built: narratives, review loop, dashboard integration, MCP server, narrative eval,
  `@claude` workflow. The version is bumped; the tag follows the squash merge.
- 170+ tests across the two venvs, ruff clean.

**Blocked on the user, not on code:**
- `ANTHROPIC_API_KEY` is not set anywhere. Until it is in `.env`, `ledgerlens narrate` and
  `ledgerlens eval-narratives` cannot run, so `docs/narrative-eval.md` still says "not yet
  run" and no narrative has been generated by the real API. Until it is in the repository
  secrets, `@claude` cannot answer.
- The Claude GitHub App must be installed on the repository for `claude.yml` to authenticate.
- Claude Desktop must be fully quit and reopened after its config gains the `ledgerlens`
  server (see README). The stdio path is proven with the SDK's own client; the Desktop
  question itself is the user's to ask.
