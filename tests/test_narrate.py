...
A fake client stands in for anthropic.Anthropic: it records every request and
replays canned responses shaped like the real ones - including the thinking
block current models return before their text.
"""
...
def _response(payload=GOOD, stop_reason="end_turn", thinking=True, usage=None, text_block=True):
    content = [_block("thinking", thinking="")] if thinking else []
    if text_block:
    ...
    return SimpleNamespace(
        ...
        usage=usage or _usage(), model=DEFAULT_MODEL,
    )


class FakeClient:
    """Records requests; replays responses in order, raising any exception given."""
...
def test_request_uses_structured_output_and_caches_the_system_prompt():
    client = FakeClient()
...
def test_text_block_is_found_after_a_thinking_block():
    narrative = Narrator(client=FakeClient([_response(thinking=True)])).narrate_one("p")
    assert narrative["summary"] == GOOD["summary"]
...
def test_response_without_text_is_an_error():
    client = FakeClient([_response(text_block=False)])
    with pytest.raises(NarrativeError, match="no text block"):
        Narrator(client=client).narrate_one("p")


def test_usage_is_accumulated_from_the_api_not_estimated():
    client = FakeClient([_response(), _response(), _response(usage=_usage(cache_read_input_tokens=0,
...
def test_parse_tolerates_a_markdown_fence():
    fenced = "```json\n" + json.dumps(GOOD) + "\n```"
...
def test_validate_rejects_broken_contracts(mutate, message):
    payload = dict(GOOD)
    mutate(payload)
...
def test_narrate_takes_the_highest_risk_first_and_honours_top_n_and_skip(ledger, scored):
    entries, flags = scored
    ...
    result = Narrator(client=FakeClient()).narrate(shuffled, flags, ledger, top_n=3, skip=skip)
...
def test_narrate_records_a_failure_without_stopping_the_batch(ledger, scored):
    entries, flags = scored
    client = FakeClient([_response(), _response(stop_reason="max_tokens"), _response()])
...
def test_auth_failure_aborts_the_batch_instead_of_failing_every_entry(ledger, scored):
    entries, flags = scored

    class AuthenticationError(Exception):
        pass

    client = FakeClient([AuthenticationError("401 invalid x-api-key")])
...
def test_format_narrative_renders_evidence_as_bullets():
    text = format_narrative(validate(GOOD))
    assert text.startswith(GOOD["summary"])
